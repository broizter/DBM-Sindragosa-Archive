...
DBM = {
	Revision = ("$Revision: 90070 $"):sub(12, -3), -- From broizter: Added extra decimal because I messed up in a earlier release.
	Version = "9.07",
	DisplayVersion = "9.07 DBM-Sindragosa by broizter", -- the string that is shown as version
	ReleaseRevision = 90070 -- the revision of the latest stable version that is available (for /dbm ver2).
}
					...
					if not showedUpdateReminder then
						...
						if found then
							...
							if not DBM.Options.BlockVersionUpdatePopup then
								...
								DBM:AddMsg(("|HDBM:update:%s:%s|h|cff3588ff[https://github.com/broizter/DBM-Sindragosa]"):format(displayVersion, revision))
							end
						end
					end
				end
...
do
	...
	function DBM:ShowUpdateReminder(newVersion, newRevision, text, url)
		urlText = url or L.UPDATEREMINDER_URL or "https://github.com/broizter/DBM-Sindragosa"
		if not frame then
			createFrame()
		else
			editBox:SetText(urlText)
